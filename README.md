...
For our first basic model we used square footage of living space (`sqft_living`) as our independant variable and home prices as our dependant variable. The relationship between `sqft_living` and `price` did not meet the criteria for linear regression, so we ran a log function on both 'price` and `sqft_living` and discovered that using 'log_price` allowed us to run a linear regression model. 

### Bedrooms and Bathrooms
We inititally tried to build off of the baseline model, but quickly discovered that due to high multicollinearity between `sqft_living` and `bathrooms` it was better to build a seperate model with `bedrooms` and `bathrooms` as the independant variables. Once again, `log_price` was our dependant varible.

### Grade and Condition
We OHE both `grade` and `condtion` as they are both quantitative variables. As such, we first added all of the grade columns to our baseline model which included `sqft_living`. Afterwards, we added in the condition columns, creating our final model.
...
Graph of predicted home prices based compared to squarefood living area: 
#ADD IN GRAPH IMAGE
### Bedrooms and Bathrooms
